...
import alphabetapool from './../../data/celo/alphabetapool.json';
...
const updateAmmPrices = async () => {
  ...
  try {
    ...
    //   console.log('NON AMM TOKEN PRICES');
    ...
    // tokenPricesCache = nonAmmPrices;
    // lpPricesCache = nonAmmPrices;
...
tokenPricesCache = init.then(({ nonAmmPrices }) => nonAmmPrices);
lpPricesCache = init.then(({ nonAmmPrices }) => nonAmmPrices);
